...
class Tmserver(commands.Cog):
    ...
    async def tms_public(self, ctx):
        ...
        e.add_field(name='登入', value=f'''**登入1**({pu['登入1']:.2f})    \t\t**登入2**({pu['登入2']:.2f})    \t\t**登入3**({pu['登入3']:.2f})    \t\t**登入4**({pu['登入4']:.2f})    \t\t**登入5**({pu['登入5']:.2f})    \t\t**登入6**({pu['登入6']:.2f})    \t\t**登入測試**({pu['登入測試']:.2f})    \t\t''', inline=False)
        e.add_field(name='跨服', value=f'''**跨服1**({pu['跨服1']:.2f})    \t\t**跨服2**({pu['跨服2']:.2f})    \t\t**跨服3**({pu['跨服3']:.2f})    \t\t**跨服4**({pu['跨服4']:.2f})    \t\t**跨服5**({pu['跨服5']:.2f})    \t\t''', inline=False)
