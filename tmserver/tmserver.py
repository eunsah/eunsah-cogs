...
class Tmserver(commands.Cog):
    ...
    async def tms_public(self, ctx):
        ...
        e.add_field(name='登入', value=rf'''**登入1** ({pu['登入1']})    **登入2** ({pu['登入2']})    **登入3** ({pu['登入3']})    **登入4** ({pu['登入4']})    **登入5** ({pu['登入5']})    **登入6** ({pu['登入6']})    **登入測試**( {pu['登入測試']})    ''', inline=False)
        e.add_field(name='跨服', value=rf'''**跨服1** ({pu['跨服1']})    **跨服2** ({pu['跨服2']})    **跨服3** ({pu['跨服3']})    **跨服4** ({pu['跨服4']})    **跨服5** ({pu['跨服5']})    ''', inline=False)
