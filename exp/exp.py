...
class Exp(commands.Cog):
    ...
    async def exp(self, ctx, *argv):
        '''
            ...
            Update exp
        '''
        await ctx.send('exp called')

        if len(argv) != 2:
            # argv check
            await ctx.send(f'Not enough arguments')
            return
        await ctx.send('exp line 97')
